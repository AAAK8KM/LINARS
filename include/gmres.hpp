...
namespace LINARS {
    ...
    class GMRESdata
    {
        ...
        GMRESdata(uint32_t v_size):basis(v_size,M),R(M,M), h(v_size+1), r(v_size+1){}
    ...
    template<uint32_t M,typename dtype, typename Mtype>
    requires IsMatrix<dtype, Mtype>
    Vector<dtype> GMRES_R(const Mtype& A, const Vector<dtype>& b, const Vector<dtype>& x0, GMRESdata<M,dtype>& D, uint32_t max_iter=preset_max_iter, dtype max_r=preset_max_r)
    {
        uint32_t n=0;
        //GMRESdata<M,dtype> D(x0.size().first);
        Vector<dtype> v=A*x0-b;
        Vector<dtype> &h=D.h, &r=D.r;
        r[0]=v.lenght();
        D.basis[0]=v/v.lenght();
        for (uint32_t i=1;i<std::min(M,h.size().first);i++)
        {
